...
class Navbar extends Component {
    ...
    render() {
        const {onScrollTop,onScrollOffer, onScrollContact, onScrollGenerator, onScrollGallery, onScrollAbout}=this.props
        return (
            <div className={styles.navbar} onClick={this.handleClick}>
                {this.menuSymbol()}
                <div className={styles.nav} style={{display:this.state.show_hide_menu}}>
                    <ul className={styles.menu}>
                        <li onClick={onScrollTop} className={styles.menu_item}>Главная</li>
                        <li onClick={onScrollOffer} className={styles.menu_item}>Предложение</li>
